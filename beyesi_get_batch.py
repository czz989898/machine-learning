            ...
            with open(r'E:\home work\semester3\machine learning\assignment\data\feature_word_data.txt','w',encoding='UTF-8') as feature_word_data:
                ...
                for i in train_data:
                    ...
                    while j <len(sentence)-1:
                        if (sentence[j].isalpha() or sentence[j].isdigit() or sentence[j] in ['!', '@', '#', '$',',', '.', '?', ':','"']) and sentence[j + 1] in ['!', '@', '#', '$',',', '.', '?', ':','"']:
    ...
    with open(r'E:\home work\semester3\machine learning\assignment\data\feature_word_data.txt', 'r',
        ...
        for i in feature_word_data:
            ...
            try:
                ...
                sentence_l = sentence.split(' ')
                for j in sentence_l:
                    if j not in words and '\\' not in j and '/' not in j and (j.isalpha() and judge_pure_english(j) or j in ['!', '@', '#', '$', '?', ':']) and 'http' not in j and ':' not in j and 1<=len(j)<20:
                        words[j]=1
                    if j in words:
                        words[j] +=1
            except:
